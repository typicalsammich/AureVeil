...
export default async function Explore({searchParams}:{searchParams:Promise<{q?:string;similar?:string}>}){
  ...
  return <div className="shell py-9">
    ...
    <div className="explore-tabs" aria-label="Explore filters">
      <a href="/explore" className={!p.q?'active':''}>For You</a>
      {['Trending','New','Photography','Illustration','Tattoo','Painting','Digital','3D','Design','Abstract','Portrait'].map(x=>
        <a key={x} href={`/explore?q=${encodeURIComponent(x)}`} className={p.q===x?'active':''}>{x}</a>
      )}
    </div>
    <ArtGrid/>
  </div>
